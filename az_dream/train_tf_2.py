...
def main(
    ...
    RANDOM_STATE=42,
    #
    cutoff_scores=[None, None, None, None],
):
    """
    """
    ...
    if test_df is not None:
        __, __, __, test_info, test_dataset, test_labels = (
            ml.get_data_for_step(train_df, test_df, pairwise_data)
        )
    else:
        test_info, test_dataset, test_labels = None, None, None
    ...
    s.tf_valid_dataset = valid_dataset
    s.tf_test_dataset = test_dataset

    print("Number of features: {}".format(s.N_COLUMNS))

    import az_dream.tf.graph_2
    reload(az_dream.tf.graph_2)
    import az_dream.tf.graph_2 as g
...
#                else:
                ...
                ch1_scores_list.append(ch1_scores)
...
#                else:
                ...
                ch2_scores_list.append(ch2_scores)
            ...
            if (step % N_STEPS_PER_CHECK) == 0 or (step == (N_STEPS - 1)):
                ...
                if ((cutoff_scores[0] is not None and cutoff_scores[0] < ch1_scores[0]) or
                    (cutoff_scores[1] is not None and cutoff_scores[1] < ch1_scores[1])):
                    print('Reached target score 1: {}'.format(cutoff_scores))
                    break
                # Accuracy
                valid_predictions_c = g.valid_prediction_c.eval(
                    feed_dict={g.tf_valid_dataset: valid_dataset})
                ...
                if ((cutoff_scores[2] is not None and cutoff_scores[2] < ch2_scores[0]) or
                    (cutoff_scores[3] is not None and cutoff_scores[3] < ch2_scores[1])):
                    print('Reached target score 2: {}'.format(cutoff_scores))
                    break
                print('')
                sys.stdout.flush()

        if test_df is not None:
            test_predictions_r = g.test_prediction_r.eval(
                feed_dict={g.tf_test_dataset: test_dataset})
            test_r = test_info.copy()
            test_r['synergy_score'] = test_labels
            test_r['synergy_score_pred'] = test_predictions_r
            test_r = (
                test_r
                .groupby(['d_1', 'd_2', 'c'])
                [['synergy_score', 'synergy_score_pred']]
            ...
            print('test_r shape: {}'.format(test_r.shape))

            test_predictions_c = g.test_prediction_c.eval(
                feed_dict={g.tf_test_dataset: test_dataset})
            test_c = test_info.copy()
            test_c['synergy_score'] = test_labels
            test_c['synergy_score_pred'] = test_predictions_c
            test_c = (
                test_c
                .groupby(['d_1', 'd_2', 'c'])
                [['synergy_score', 'synergy_score_pred']]
            ...
            print('test_c shape: {}'.format(test_c.shape))
        else:
            test_r, test_c = None, None

    return ch1_scores_list, ch2_scores_list, tmp_r, tmp_c, test_r, test_c
...
if __name__ == '__main__':
    ...
    if opts['RANDOM_STATE'] is None:
        ...
        for i in range(3):
            opts['RANDOM_STATE'] = i
            scores1, scores2, tmp_r, tmp_c, test_r, test_c = main(
                df_train, df_validate, pairwise_data, **opts)
            scores = sorted(scores1)[-1] + sorted(scores2)[-1]
    ...
    while n_tries < 5:
        try:
            engine = sa.create_engine(
                '{}/az_dream_2015_ml'.format(os.environ['BIODB_CONNECTION_STG']))
            df.to_sql(args['DB_TABLE'], engine, if_exists='append', index=False)
            break
