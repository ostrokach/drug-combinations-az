...
def get_best_metric(data, columns, scores, feature_subset_name, row_subset_name=None):
    ...
    results_df.sort_values('corr_abs', ascending=False, inplace=True)
    return results_df
...
def loo_groupby(df, by, features, stats, name=''):
    ...
    results_df = pd.DataFrame(results, index=df.index, columns=columns)
...
def get_synergy_groups(df, df_test=None, pairwise=False):
    """
    """
    STATS = ['min', 'max', 'mean', 'std']
    if df_test is None:
...
# %% Scores for Subchallenge 1 and Subchallenge 2
def get_score_ch1(df):
    """
    ...
    results_df = pd.DataFrame(results, columns=by + ['count', 'synergy_score_max', 'pearson_r'])
    ...
    results_df_high = results_df[results_df['synergy_score_max'] > 20]
    tiebreak_metric = (
