...
getGlobalScore_ch1 <- function(obs, pred) {
  ...
  R      <- ddply(
    temp, "COMBINATION_ID",
    function(x) {
      if( length(x$OBSERVATION) > 1) {
        return(cor(x$OBSERVATION,x$PREDICTION))
      } else {
        return(0)
      }
    })
  ...
  print(c("top", sum(R$V1*sqrt(R$N-1))))
  print(c("bottom", sum(sqrt(R$N-1))))
  print(DrugCombiScore_ch1_Wsq)
  DrugCombiScore_ch1_GT20  <- mean(R[obsMax[,2] >= 20,2])          # tie-breaking metric
  print(DrugCombiScore_ch1_GT20)
