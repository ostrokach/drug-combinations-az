-- THIS FILE RUNS WAY TOO SLOW!!! --
-- Use python instead --

--
drop table if exists drug_pair_to_ensp_pair;
...
ep.ensp_1 ensp_1,
ep.ensp_2 ensp_2
FROM all_drug_pairs dp
join drug_to_target t1 on (t1.drug = dp.compound_a)
join drug_to_target t2 on (t2.drug = dp.compound_b)
join ensp_to_ensp_pair ep1 on (ep1.ensp = t1.ensp)
join ensp_to_ensp_pair ep2 on (ep2.ensp = t2.ensp and ep1.ensp_pair = ep2.ensp_pair)
join ensp_pairs ep on (ep.ensp_pair = ep2.ensp_pair);
...
--
drop table if exists drug_pair_to_ensp_idx_pair;
...
insert into if exists drug_pair_to_ensp_idx_pair
SELECT
unique_id,
...
join drug_to_target t2 on (t2.drug = dp.compound_b)
join ensp_to_ensp_pair ep1 on (ep1.ensp = t1.ensp)
join ensp_to_ensp_pair ep2 on (ep2.ensp = t2.ensp and ep1.ensp_idx_pair = ep2.ensp_idx_pair)
join ensp_idx_pairs ep on (ep.ensp_idx_pair = ep2.ensp_idx_pair);
...
--
drop table if exists drug_pair_to_gene_pair;
...
join drug_to_target t1 on (t1.drug = dp.compound_a)
join drug_to_target t2 on (t2.drug = dp.compound_b)
join hgnc_to_hgnc_pair ep1 on (ep1.gene_name = t1.hgnc_name)
join hgnc_to_hgnc_pair ep2 on (ep2.gene_name = t2.hgnc_name and ep2.gene_pair = ep1.gene_pair)
join hgnc_pairs ep on (ep.gene_pair = ep2.gene_pair);
